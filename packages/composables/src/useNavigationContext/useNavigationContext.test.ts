...
describe("useNavigationContext", () => {
  ...
  it("should return empty string when context is null", () => {
    const { vm } = useSetup(() => useNavigationContext(ref(null)));
    expect(vm.foreignKey).toBe("");
    expect(vm.routeName).toBeUndefined();
    expect(vm.navigationContext).toBeNull();
  });
});
