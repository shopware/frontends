...
The information about the session is kept in a cookie (`sw-context-token`) and it's used in every request made by any composable or directly, invoked by `apiClient instance`:

```html
<script>
  const { apiClient } = useShopwareContext();
  const response = await apiClient.invoke(/** params omitted */);
</script>
```
