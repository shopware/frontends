...
Shopware Frontends is a framework and not a finished product. Even it the [demo store template](./templates/demo-store-template.html) looks like a proper store, it's not an off-the-shelf solution.
...
<PageRef page="./templates.html" title="Getting Started" sub="Get started with Shopware Frontends" />
