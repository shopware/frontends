...
const languagesQuery = useAsyncData("languages", () => getAvailableLanguages());
const currenciesQuery = useAsyncData("storeCurrencies", async () => {
...
Three choices in this example look unusual and are deliberate. The patch and the navigation sit in **two** `try` blocks, because once `changeLanguage()` resolves the context has already moved: a throw after that point means the language changed and the page did not, and saying "the switch failed" would be a lie. The selects carry `aria-disabled` rather than `disabled`, because a disabled control cannot hold focus and a keyboard user would be thrown to the top of the document — `aria-disabled` does not block activation, so each handler opens with its own `isSwitchingContext` guard. And the bindings are `:value` on the `<select>` rather than `:selected` on each `<option>`, because Vue force-syncs only the prop named `value`; with `:selected`, a failed switch leaves the control showing a pick the context never accepted.
...
The example owns its initial load. `vue-starter-template` already fetches both lists in `app.vue` and `useCurrencySwitcher`, and these `useAsyncData` keys match its own, so inside that template the calls dedupe rather than fetching twice on hydration.
