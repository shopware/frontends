...
import { useProductSearchSuggest } from "../../composables/useProductSearchSuggest";
withDefaults(
  defineProps<{
    displayTotal?: number;
  }>(),
...
// TODO: please replace with your translation function
const $t = (key: string, plurar?: number | string) => key;
  ...
  >
    ...
    >
      ...
      <div class="bg-white p-4">
        <div
          class="p-4 text-secondary-600 border-t-secondary-200 border-t-1"
          v-if="getCategories.length"
        >
          Categories
          <ul
            class="flex flex-wrap mt-2 list-none items-center justify-center text-gray-800 dark:text-white"
          >
            <li v-for="category in getCategories" :key="category.id">
              <NuxtLink
                ...
                data-testid="layout-search-suggest-link"
                @click="[(active = false), $emit('link-clicked')]"
                >{{ category.translated.name }}</NuxtLink
              >
            </li>
          </ul>
        </div>
        ...
        >
          Suggestions
          <ul
            class="flex flex-wrap mt-2 list-none items-center justify-center text-gray-800"
          >
            <li v-for="phrase in getSuggestions" :key="phrase">
              <NuxtLink
                class="me-4 hover:underline md:me-6"
                :to="{ path: `/search`, query: { search: phrase } }"
                >{{ phrase }}</NuxtLink
              >
            </li>
          </ul>
      ...
      <div
        ...
        :class="[loading ? ['bg-primary'] : ['bg-gray-100']]"
      >
        <div
          v-if="loading"
        ...
        <div v-else>
          <div v-if="getSuggestions.length === 0">
            <NuxtLink
              ...
              :to="
                { path: `/search`, query: { search: typingQuery } }
              "
              @click="[(active = false), $emit('link-clicked')]"
            >
              {{ $t("search.see") }}
